...
ROOT.gROOT.ProcessLine('#include "%s/src/FastPUPPI/NtupleProducer/python/scripts/jetHtSuite_new.h"' % os.environ['CMSSW_BASE']);
...
def makeGenArray(tree, what, ptCut, etaCut,sigCut):
    return makeCorrArray(tree, what, "Gen", ptCut, etaCut, sigCut, None)

def makeCorrArray(tree, what, obj, ptCorrCut, etaCut, sigCut, corr, _cache={}):
    _key = (id(tree),what,obj,int(ptCorrCut*100),int(etaCut*1000))
    if _key in _cache: return _cache[_key]
    if what == "metmht":
        met = makeCorrArray(tree, "met", obj,     0,      5.0, 0,  corr, _cache=_cache)
        mht = makeCorrArray(tree, "mht", obj, ptCorrCut, sigCut, etaCut, corr, _cache=_cache)
        ret = ROOT.makeMinimum(met,mht)
        _cache[_key] = ret
        return ret
    if "met" in what:
        ret = makeMETArray(tree, what, obj, etaCut, sigCut)
        _cache[_key] = ret
        return ret
...
def makeMETArray(tree, what, obj, etaCut, sigCut):
    if obj == "Gen": obj = "gen" # fix issue with naming convention
    if   etaCut <= 1.5: post = "MetBarrel" 
    elif etaCut <= 2.4: post = "MetCentral"
    else:               post = "Met"
    if   etaCut <= 1.5: post_sig = "MetBarrel" 
    elif etaCut <= 2.4: post_sig = "MetCentral"
    else:               post_sig = "L1PuppiMet"
    if not tree.GetBranch(obj+post+"_pt"): 
        if obj == "gen": raise RuntimeError("Missing gen"+post);
        return None
    if not tree.GetBranch(post_sig+"_sig"): 
        raise RuntimeError("Missing significance branch");
        return None
    progress = _progress("  Reading "+obj+post+" in C++...")
    ret = ROOT.makeMetArray(tree, obj+post, post_sig, sigCut);
    progress.done("done, %d entries" % len(ret))
    return ret

def makeCumulativeHTEff(name, corrArray, xmax, norm=2760.0*11246/1000):
    ret, makenew = makeCumulativeHTEffGenCut(name, corrArray, None, None, xmax, norm)
    return ret, makenew
...
parser.add_option("-S", "--sigCut", dest="sig",  default=4, type="float", help="Choose significance cut")
...
def makePlatEffPlot(signal, background, what, obj, ptcut, jecs, plotparam, _cache={}):
   ...
   # recoArrayB = makeCorrArray(background, what, obj, ptcut, options.eta, jecs)
    recoArrayB = makeCorrArray(background, what, obj, ptcut, options.eta, options.sig, jecs)
    if not recoArrayB: return (None, None)
    rateplot = makeCumulativeHTEff(name, recoArrayB, options.xmax)
    recoArrayS = makeCorrArray(signal, what, obj, ptcut, options.eta, options.sig, jecs)
...
for plotkind in options.plots.split(","):
  ...
  if plotkind != "rate":
    genArray = makeGenArray(signal, what, options.pt, options.eta, options.sig)
  if plotkind == "isorate":
      ...
      for objset,things in whats:
          ...
          for name,obj,col,msty,msiz in things:
              ...
              if plotkind == "rate":
                  ...
                  effsig, listsig= makeCumulativeHTEffGenCut(name+"_s", recoArrayS, genArray, options.genht, options.xmax, norm=1)
                  ratebkg, listbkg = makeCumulativeHTEff(name+"_b", recoArrayB, options.xmax)
                  for ix in range(len(listsig)):
                    print(listsig[ix])
