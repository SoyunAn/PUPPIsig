...
std::vector<float> makeMetArray(TTree *tree, const std::string & obj, const std::string & obj_sig, float sigCut) {
    std::vector<float> ret; 
    ret.reserve(tree->GetEntries());

    TTreeReader reader(tree);
    TTreeReaderValue<float> met_pt(reader, (obj+"_pt").c_str());
    TTreeReaderValue<float> met_si(reader, (obj_sig+"_sig").c_str());

    while (reader.Next()) {
     	if(*met_si > sigCut) ret.push_back(*met_pt);
    }
    return ret;
}
